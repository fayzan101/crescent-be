...
function buildDescription(): string {
  return [
    ...
    '',
    '### Sales workflow',
    '- `POST /api/v1/sales` — create sale + initial stage rows.',
    '- `GET /api/v1/sales`, `GET /api/v1/sales/:id` — list / detail (needs `sales.view`).',
    '- `GET /api/v1/sales/:id/audit` — audit trail (`sales.audit.view`).',
    '- **Stage updates (PATCH):**',
    '  - `/api/v1/sales/:id/sales-stage` — client + product fields; optional submit to accounts.',
...
export function buildOpenApiDocument() {
  ...
  for (const tag of SWAGGER_TAG_ORDER) {
    const descriptions: Record<string, string> = {
      ...
      Sales: 'Sale lifecycle, stage PATCH endpoints, audit — JWT + permission codes',
